...
class RoutingTestNew extends FlatSpec with Matchers with StrictLogging {
  ...
  private val microservice = NioMicroserviceMock(SignatureVerifierMicroserviceNew(c => new DefaultProtocolVerifier(keyServerClient(c))))
